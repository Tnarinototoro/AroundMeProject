...
def filter_actors_to_export(context, actors):
    """ Filters the provided list of actors to export according to `context`'s options

    :param context: UsdExportContext object describing the export
    :param actors: Input iterable of unreal.Actor objects to filter
    :returns: List of unreal.Actor objects that are approved for export
    """
    ...
    for actor in actors:
        ...
        # We only want to export actors if their (parent) persistent world or their actual world is what is being exported.
        # This is the only thing preventing us from e.g. selecting some actor in the current world, exporting some entirely
        # different world, and seeing those actors also exported
        if persistent_world != context.world and actual_world != context.world:
            continue
...
def assign_static_mesh_component_assets(component, prim, exported_assets):
    ...
    unreal.UsdConversionLibrary.add_reference(
        str(prim.GetStage().GetRootLayer().realPath),
        str(prim.GetPrimPath()),
        asset_file_path
    )


def assign_hism_component_assets(component, prim, context, material_overrides_layer, converter):
    ...
    asset_file_path = context.exported_assets[mesh]
    if asset_file_path is None:
        unreal.log_warning(f"Failed to find an exported usd file to use for mesh asset '{mesh.get_name()}'")
        return
    ...
    exported_filepath = context.exported_assets[mesh]
    ...
    proxy_prim_path = prototypes.GetPath().AppendPath(proxy_prim_name)
    ...
    try:
        ...
        converter.convert_material_overrides(
            component.get_editor_property('static_mesh'),
            component.get_editor_property('override_materials'),
            str(proxy_prim_path),
            context.options.inner.asset_options.lowest_mesh_lod,
            context.options.inner.asset_options.highest_mesh_lod
        )
    finally:
...
def assign_instanced_foliage_actor_assets(actor, prim, context, material_overrides_layer, converter):
    ...
    for foliage_type in types:
        source = foliage_type.get_source()
        if isinstance(source, unreal.StaticMesh) and source in context.exported_assets:
    ...
    for foliage_type, source in zip(exported_types, exported_type_sources):
        exported_filepath = context.exported_assets[source]
        ...
        # Don't use 'Mesh' if we have LODs because it makes it difficult to parse back the prototypes if
        # they're nested Meshes
        proxy_prim_schema = 'Mesh' if static_mesh_editor_subsytem.get_lod_count(source) < 2 else 'Xform'

        proxy_prim_path = prototypes.GetPath().AppendPath(proxy_prim_name)
        proxy_prim = stage.DefinePrim(proxy_prim_path, proxy_prim_schema)
        unreal.UsdConversionLibrary.add_reference(
            str(stage.GetRootLayer().realPath),
            str(proxy_prim_path),
            exported_filepath
        ...
        # Set material overrides if we have any
        # Careful here: The converter stage is not the same as context.stage, so we must set the context's edit target
        if isinstance(foliage_type, unreal.FoliageType_InstancedStaticMesh):
            old_edit_target = converter.get_edit_target()
            try:
                converter.set_edit_target(unreal.FilePath(material_overrides_layer.realPath))
                converter.convert_material_overrides(
                    foliage_type.get_editor_property('mesh'),
                    foliage_type.get_editor_property('override_materials'),
                    str(proxy_prim_path),
                    context.options.inner.asset_options.lowest_mesh_lod,
                    context.options.inner.asset_options.highest_mesh_lod
                )
...
def convert_component(context, converter, component, visited_components=set(), parent_prim_path=""):
    ...
    if isinstance(component, unreal.HierarchicalInstancedStaticMeshComponent):
        ...
        assign_hism_component_assets(component, child_prim, context, material_overrides_layer, converter)
    ...
    if isinstance(component, unreal.SceneComponent):
        ...
        if isinstance(owner_actor, unreal.InstancedFoliageActor):
            assign_instanced_foliage_actor_assets(actor, prim, context, material_overrides_layer, converter)
        ...
        # Export other stage actors with simple references to the stages that they're loading, if any
        elif isinstance(owner_actor, unreal.UsdStageActor):
            opened_root_layer = owner_actor.get_editor_property("root_layer")
            if opened_root_layer and opened_root_layer.file_path:

                # The only situation in which the root layer can be a relative path is if it's relative to the project
                # dir, so convert it back to absolute instead because it is unlikely this relative path will make
                # sense for the exported layer
                file_path = opened_root_layer.file_path
                if not os.path.isabs(file_path):
                    file_path = os.path.join(unreal.Paths.project_dir(), file_path)

                prim.GetPayloads().AddPayload(file_path)

                # Ideally we'd never author a type name every time we rely on reference/payloads to bring in
                # "most" of the prim's definition, but that's likely too big a change for now so here we just
                # make sure our prim can become whatever the defaultPrim of `opened_root_layer` is, without
                # conflict
                prim.ClearTypeName()
        else:
            # Recurse to children
...
def export_level(context, actors):
    ...
    # Fetch the current version of this level
    current_version_string = unreal.UsdConversionLibrary.generate_object_version_string(
        context.world,
        context.options
    )

    # Check if we actually need to export this level
    if not context.options.re_export_identical_levels_and_sequences and os.path.exists(context.root_layer_path) and len(current_version_string) > 0:
        with UsdConversionContext(context.root_layer_path) as converter:
            stage = Usd.Stage.Open(context.root_layer_path)
            root_prim = stage.GetDefaultPrim()
            existing_info = converter.get_prim_asset_info(str(root_prim.GetPrimPath()))

            version_matches = len(existing_info.version) > 0 and existing_info.version == current_version_string
            asset_type_matches = len(existing_info.unreal_asset_type) > 0 and existing_info.unreal_asset_type == str(context.world.get_class().get_name())

            # For some reason we need to manually clear our reference to the stage here, or else the
            # CreateNew call below will run into trouble as the layers still exist on the registry
            stage = None
            if version_matches and asset_type_matches:
                unreal.log(f"Skipping export of level '{context.world.get_path_name()}' as the target file '{context.root_layer_path}' already contains up-to-date exported data.")
                return
    ...
    with unreal.ScopedSlowTask(3, f"Exporting main root layer") as slow_task:
        ...
        # Write assetInfo metadata
        with UsdConversionContext(context.root_layer_path) as converter:
            info = unreal.UsdUnrealAssetInfo()
            info.name = context.world.get_name()
            info.identifier = context.root_layer_path
            info.version = current_version_string
            info.unreal_content_path = context.world.get_path_name()
            info.unreal_asset_type = context.world.get_class().get_name()
            info.unreal_export_time = datetime.now().strftime("%Y.%m.%d-%H.%M.%S")
            info.unreal_engine_version = unreal.SystemLibrary.get_engine_version()

            converter.set_prim_asset_info(str(root_prim.GetPrimPath()), info)
...
def export_material(context, material):
    ...
    options.re_export_identical_assets = context.options.re_export_identical_assets
...
def export_landscape(context, landscape_actor):
    ...
    with Usd.StageCacheContext(cache):
        ...
        if context.options.inner.asset_options.use_payload:
            ...
            unreal.UsdConversionLibrary.add_reference(
                str(mat_proxy_prim.GetStage().GetRootLayer().realPath),
                str(mat_proxy_prim.GetPrimPath()),
                mat_path
            )
...
def export(context):
    ...
    if not unreal.UsdConversionLibrary.can_export_to_layer(context.root_layer_path):
        return

    num_levels = unreal.UsdConversionLibrary.get_num_levels_to_export(context.world, context.options.inner.levels_to_ignore)
    if num_levels == 0:
        unreal.log(f"Cancelling level export of asset '{context.world.get_path_name()}' as input options don't specify any level to export. (Did you forget to check 'Persistent Level'?)")
        return
    ...
    # Get selected actors before we call stream_in_required_levels, because that will force flush level
    # streaming, which can call RemoveFromWorld and eventually end up deselecting actors.
    # Note that if we're selecting actors we can gather actors here as we know all the actors we need must be
    # loaded already (as they're selected). If we're exporting all actors however we need to do that after
    # streaming in the required sublevels, so all actors are ready
    actors = None
    ...
    try:
        ...
        with unreal.ScopedSlowTask(6, f"Exporting level to '{context.root_layer_path}'") as slow_task:
            ...
            slow_task.enter_progress_frame(1)
            if not context.options.inner.selection_only:
                actors = unreal.UsdConversionLibrary.get_actors_to_convert(context.world)
            actors = filter_actors_to_export(context, actors)
        ...
        # Our stream_in and stream_out calls will clear the user actor selection due to the FlushLevelStreaming
        # calls, so let's try restoring it
        if actors and context.options.inner.selection_only:
