...
class ReplayBuffer:
    ...
    def add_batch(self, batch: Batch):
        ...
        self._size = min(self._size + steps, self.maxsize)
        
        if self._index + steps > self.maxsize:
            split = self.maxsize - (self._index + steps)
            
            self._index = (self._index + steps) % self.maxsize
            self._ep_idx = self._index
            self.last_index[0] = (self._index + steps - 1) % self.maxsize
            
            self._meta[start:start+split] = batch[:split]
            self._meta[0:steps-split] = batch[split:]
        else:
            self._index = self._index + steps
            self._ep_idx = self._index
            self.last_index[0] = self._index + steps - 1
            self._meta[start:start+steps] = batch
