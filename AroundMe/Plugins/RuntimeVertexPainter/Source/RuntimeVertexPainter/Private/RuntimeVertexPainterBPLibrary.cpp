...
bool URuntimeVertexPainterBPLibrary::BeginPaintVertexColor(UStaticMeshComponent* StaticMeshComponent,
...
void URuntimeVertexPainterBPLibrary::EndPaintVertexColor(FRVPContext Context)
{
...
void URuntimeVertexPainterBPLibrary::ContextPaintVertexColorInShape(
	UPARAM(ref)FRVPContext& Context, FVector Position, FVector Radius, FRotator Rotation,EPaintShape PaintShape,FLinearColor Color, float Opacity,
	UCurveFloat* FallOff, bool bConvertToSRGB)
 {
	...
	if (!InMesh->bAllowCPUAccess)
	{
		FMessageLog("PIE").Warning()
			->AddToken(FTextToken::Create(FText::FromString(FString(TEXT("Try to read mesh")))))
			->AddToken(FUObjectToken::Create(InMesh))
			->AddToken(FTextToken::Create(FText::FromString(FString(TEXT("but 'Allow CPU Access' is not enabled. This is required for paint vertex color in shape when packed")))));
		FMessageLog("PIE").Warning()
		    ->AddToken(FTextToken::Create(FText::FromString(FString(TEXT("To fix it,you should find this:")))))
		    ->AddToken(FUObjectToken::Create(InMesh))
			->AddToken(FTextToken::Create(FText::FromString(FString(TEXT("and search 'Allow CPU Access' in detail panel,make it ture")))));
	...
	for (FVertexColorsArray& VertexColorsArray : Context.VertexColorsArray)
	{
		...
		const FPositionVertexBuffer& PositionVertexBuffer = Context.StaticMeshComponent->GetStaticMesh()->GetRenderData()->
		...
		for (uint32 VertexIndex = 0; VertexIndex < NumVertices; VertexIndex++)
		{
			...
			case EPaintShape::Sphere : distance = UKismetMathLibrary::Vector_Distance(LocalPosition,FVector::ZeroVector);
				break;
				//If Bos,use max axis value
			case EPaintShape::Box : distance = LocalPosition.GetAbsMax();
				break;
				default: distance = 2;
			}
...
void URuntimeVertexPainterBPLibrary::ContextPaintAllVerticesColor(FRVPContext& Context, FLinearColor Color, float Opacity, bool bConvertToSRGB)
{
	if (!Context.StaticMeshComponent || !Context.StaticMeshComponent->GetStaticMesh())
	{
		return;
	...
	for (FVertexColorsArray& VertexColorsArray : Context.VertexColorsArray)
	{
		...
		for (uint32 VertexIndex = 0; VertexIndex < NumVertices; VertexIndex++)
		{
				...
				switch (static_cast<int32>(Context.Channel))
				{
				case 0:FinalColor = UKismetMathLibrary::LinearColorLerp( BaseColor, Color, Opacity);
					break;
					
				case 1:FinalColor =UKismetMathLibrary::MakeColor(UKismetMathLibrary::Lerp(BaseColor.R,Color.R,Opacity),BaseColor.G,BaseColor.B,BaseColor.A);
					break;
					
				case 2:FinalColor =UKismetMathLibrary::MakeColor(BaseColor.R,UKismetMathLibrary::Lerp(BaseColor.R,Color.G,Opacity),BaseColor.B,BaseColor.A);
					break;
					
				case 3:FinalColor =UKismetMathLibrary::MakeColor(BaseColor.R,BaseColor.G,UKismetMathLibrary::Lerp(BaseColor.R,Color.B,Opacity),BaseColor.A);
					break;
					
				case 4:FinalColor =UKismetMathLibrary::MakeColor(BaseColor.R,BaseColor.G,BaseColor.B,UKismetMathLibrary::Lerp(BaseColor.R,Color.A,Opacity));
					break;
					
				default:FinalColor = FLinearColor::Black;
				}
...
bool URuntimeVertexPainterBPLibrary::BeginPaintSkeletalMeshVertexColor(USkeletalMeshComponent* SkeletalMeshComponent,
...
void URuntimeVertexPainterBPLibrary::EndPaintSkeletalMeshVertexColor(FRVPContextSK ContextSK)
{
	if (!ContextSK.SkeletalMeshComponent || !ContextSK.SkeletalMeshComponent->SkeletalMesh)
	{
		UE_LOG(LogTemp, Warning, TEXT("No Skeletal Mesh Found"));
		return;
	}
...
void URuntimeVertexPainterBPLibrary::ContextPaintSkeletalMeshVertexColorInShape(
	FRVPContextSK& ContextSK, FVector Position, FVector Radius, FRotator Rotation,EPaintShape PaintShape, FLinearColor Color, float Opacity, UCurveFloat* FallOff,
	bool bConvertToSRGB)
{
	if (!ContextSK.SkeletalMeshComponent || !ContextSK.SkeletalMeshComponent->SkeletalMesh)
	{
	...
	{
		//Get vertices position information
		FSkeletalMeshLODRenderData& LODRenderData = ContextSK.SkeletalMeshComponent->SkeletalMesh->GetResourceForRendering()
		...
		for (uint32 VertexIndex = 0; VertexIndex < NumVertices; VertexIndex++)
		{
			...
			if (distance <= 1)
			{
				...
				{
				case 0:FinalColor = UKismetMathLibrary::LinearColorLerp( BaseColor, Color, alpha);
					break;
					
				case 1:FinalColor =UKismetMathLibrary::MakeColor(UKismetMathLibrary::Lerp(BaseColor.R,Color.R,alpha),BaseColor.G,BaseColor.B,BaseColor.A);
					break;
					
				case 2:FinalColor =UKismetMathLibrary::MakeColor(BaseColor.R,UKismetMathLibrary::Lerp(BaseColor.R,Color.G,alpha),BaseColor.B,BaseColor.A);
					break;
					
				case 3:FinalColor =UKismetMathLibrary::MakeColor(BaseColor.R,BaseColor.G,UKismetMathLibrary::Lerp(BaseColor.R,Color.B,alpha),BaseColor.A);
					break;
					
				case 4:FinalColor =UKismetMathLibrary::MakeColor(BaseColor.R,BaseColor.G,BaseColor.B,UKismetMathLibrary::Lerp(BaseColor.R,Color.A,alpha));
					break;
...
void URuntimeVertexPainterBPLibrary::ContextPaintAllSkeletalMeshVerticesColor(FRVPContextSK& ContextSK, FLinearColor Color, float Opacity, bool bConvertToSRGB)
{
	if (!ContextSK.SkeletalMeshComponent || !ContextSK.SkeletalMeshComponent->SkeletalMesh)
	{
		return;
	...
	for (FVertexColorsArray& VertexColorsArray : ContextSK.VertexColorsArray)
	{
		...
		const uint32 NumVertices = GetSkeletalMeshVerticesNum(ContextSK.SkeletalMeshComponent->SkeletalMesh,LODIndex);
	...
	USkeletalMeshComponent* SkeletalMeshComponent, int32 LODIndex)
{
	...
	int32 LODNun = SkeletalMeshComponent->SkeletalMesh->GetLODNum();
	if (!SkeletalMeshComponent->SkeletalMesh->GetResourceForRendering()->LODRenderData.IsValidIndex(LODIndex))
	{
		return  VertexColors;
	}
	
	const FSkeletalMeshLODRenderData& MeshLODResources = SkeletalMeshComponent->SkeletalMesh->
					GetResourceForRendering()->LODRenderData[LODIndex];
...
TArray<FVertexColorsArray> URuntimeVertexPainterBPLibrary::SkeletalMeshGetVertexColorsInAllLOD(USkeletalMeshComponent* SkeletalMeshComponent)
{
	...
	int32 NumLODs = SkeletalMeshComponent->SkeletalMesh->GetLODNum();
	FVertexColorsArray VertexColors;
	for (int32 LODindex = 0;LODindex<NumLODs;LODindex++)
	{
		VertexColors.VertexColors = SkeletalMeshGetVertexColorsInLOD(SkeletalMeshComponent,LODindex);
