...
void UDragonIKPhysicsComponent::Server_OnHitCallback_Implementation(const FHitResult& Hit,const 
	FName Optional_Override_Hit_Bone_Name,const  FVector& hit_location,const  FVector& hit_impulse_direction,const  FName& hit_bone_name)
{
	Multicast_OnHitCallback(Hit,Optional_Override_Hit_Bone_Name,hit_location,hit_impulse_direction,hit_bone_name);
}

bool UDragonIKPhysicsComponent::Server_OnHitCallback_Validate(const FHitResult& Hit,const 
	FName Optional_Override_Hit_Bone_Name,const  FVector& hit_location,const  FVector& hit_impulse_direction,const  FName& hit_bone_name)
{
	return true;
}

void UDragonIKPhysicsComponent::Multicast_OnHitCallback_Implementation(const FHitResult& Hit,const 
	FName Optional_Override_Hit_Bone_Name,const  FVector& hit_location,const  FVector& hit_impulse_direction,const  FName& hit_bone_name)
{
	FVector Hit_Impulse_Var = hit_location;
	FVector Hit_Dir_Var = hit_impulse_direction;
	FName Hit_Bone_Name_Var = hit_bone_name;
	
	OnHitCallback(Hit,Optional_Override_Hit_Bone_Name,Hit_Impulse_Var,Hit_Dir_Var,Hit_Bone_Name_Var);
}

bool UDragonIKPhysicsComponent::Multicast_OnHitCallback_Validate(const FHitResult& Hit,const 
	FName Optional_Override_Hit_Bone_Name,const  FVector& hit_location,const  FVector& hit_impulse_direction,const  FName& hit_bone_name)
{
	return true;
}
...
/////////////////// TELEPORTATION LOGIC
			...
			if(!Weapon_Delta_Transform.Equals(FTransform::Identity) && (Weapon_Delta_Transform.GetLocation() - owning_skeleton->GetComponentTransform().GetLocation()).Length() > Teleport_Distance)
			{
			...
			Weapon_Delta_Transform = owning_skeleton->GetComponentTransform();
