...
void UDragonIK_Library::EstablishParentChildRelationship(FTransform& Transform1, const FTransform& InitialTransform1, FTransform& Transform2, const FTransform& InitialTransform2)
{
	// Calculate the relative transformation between Transform2 and Transform1
	FTransform RelativeTransform = Transform2.GetRelativeTransform(Transform1);

	// Apply the relative transformation to Transform2
	Transform2 = RelativeTransform;
}
...
//	FTransform EndBoneCSTransform = Output.Pose.GetComponentSpaceTransform(IKBoneCompactPoseIndex) * transform_offset;
	...
	*/
	...
	FTransform mesh_transform_zeroed = arm_ik_params.mesh_component_transform;
	//mesh_transform_zeroed.SetRotation(FRotator(0,180,0).Quaternion());
	mesh_transform_zeroed.SetRotation(FQuat::Identity);
	//Forward_Rotation_Difference

	//GEngine->AddOnScreenDebugMessage(-1, 0.01f, FColor::Red, " LookAt_Axis_Input : "+FRotator(Forward_Rotation_Difference).ToString()+" -- ");

//	GEngine->AddOnScreenDebugMessage(-1, 0.01f, FColor::Red, " LookAt_Axis_Input : "+arm_ik_params.LookAt_Axis_Input.ToString()+" -- "+arm_ik_params.Reference_Constant_Forward_Axis_Input.ToString());
	
	FTransform FRP_Knee_Transform = mesh_transform_zeroed;
...
//OLD1	JointTargetTransform.SetLocation(JointTargetTransform.GetLocation() + Pole_Transform.GetLocation());
	...
	//JointTargetTransform = JointTargetTransform*(FRP_Knee_Transform*mesh_transform_zeroed.Inverse());

	JointTargetTransform = JointTargetTransform*(mesh_transform_zeroed*FRP_Knee_Transform.Inverse());
...
bool UDragonIK_Library::TraceSelectorFire(UDragonIKTraceManagerComponent* TraceComponent,UObject* WorldContextObject, FVector startpoint, FVector endpoint,float trace_radius,bool is_line,
	ETraceTypeQuery TraceChannel, bool bTraceComplex, const TArray<AActor*>& ActorsToIgnore,
	EDrawDebugTrace::Type DrawDebugType, FHitResult& OutHit, bool bIgnoreSelf, FLinearColor TraceColor,FName unique_identifier,FName bone_text)
{

	FDragonIKTraceMParams trace_params = FDragonIKTraceMParams();
	trace_params.StartLocation = startpoint;
	trace_params.EndLocation = endpoint;
	
	trace_params.ChannelInput = TraceChannel;
	trace_params.SphereRadius = trace_radius;
	trace_params.is_line_type = is_line;
	trace_params.bone_text = bone_text;

	if(is_line)
	  trace_params.TraceType = EDragonIKTraceMType::LineTrace;
	else
		trace_params.TraceType = EDragonIKTraceMType::SphereTrace;

	
	trace_params.TraceIdentifier = unique_identifier;
	//const bool bHit = UKismetSystemLibrary::LineTraceSingle(WorldContextObject, startpoint, endpoint, TraceChannel, bTraceComplex, ActorsToIgnore, DrawDebugType, OutHit, bIgnoreSelf, TraceColor);

	if(TraceComponent != nullptr)
	{
		TraceComponent->hand_shaked = true;
		TraceComponent->IssueTraceInput(trace_params);
		//trace_params.hit_result.ImpactPoint = FVector(19,10,10);
		OutHit = trace_params.hit_result;

		//GEngine->AddOnScreenDebugMessage(-1, 0.01f, FColor::Red, " ImpactNormal " + trace_params.hit_result.ImpactNormal.ToString()+" and "+FString::SanitizeFloat(TraceComponent->HitResultMap.Array().Num()));
