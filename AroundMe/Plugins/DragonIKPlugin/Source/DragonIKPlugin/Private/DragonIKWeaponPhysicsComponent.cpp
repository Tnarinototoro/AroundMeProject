	...
	if (is_grabbing && weapon_physics_handle != nullptr && weapon_physics_handle != NULL && owning_skeleton != nullptr)
	{
		...
		if(active_weapon_component != nullptr)
		{
			...
			//	EnableWeaponsArms();
				...
				active_weapon_component->SetWorldTransform(Saved_Relative_Hand_Transform*owning_skeleton->GetSocketTransform(weapon_grabbed_bone,ERelativeTransformSpace::RTS_World));
	...
	if (owning_skeleton->GetBoneIndex(weapon_grabbed_bone) > -1)
	{
		...
		*/
		...
		//	active_weapon_component->SetWorldLocation( (dominant_shoulder.GetLocation() + weapon_hand_offset));
			active_weapon_component->SetWorldLocation( (dominant_shoulder.GetLocation()));
...
void UDragonIKWeaponPhysicsComponent::Grab_Weapon(FName grabbed_bone,bool simulate_physics,bool use_gravity)
{
	/*
...
void UDragonIKWeaponPhysicsComponent::Server_Grab_Weapon_Implementation(FName grabbed_bone, bool simulate_physics, bool use_gravity)
{
	Multicast_Grab_Weapon(grabbed_bone,simulate_physics,use_gravity);
}

bool UDragonIKWeaponPhysicsComponent::Server_Grab_Weapon_Validate(FName grabbed_bone, bool simulate_physics, bool use_gravity)
{
	return true;
}

void UDragonIKWeaponPhysicsComponent::Multicast_Grab_Weapon_Implementation(FName grabbed_bone, bool simulate_physics, bool use_gravity)
{
	Grab_Weapon(grabbed_bone,simulate_physics,use_gravity);
}

bool UDragonIKWeaponPhysicsComponent::Multicast_Grab_Weapon_Validate(FName grabbed_bone, bool simulate_physics, bool use_gravity)
{
	return true;
}
