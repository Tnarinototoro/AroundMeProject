...

			{
				...
				if (spine_hit_pairs[spine_index].RV_Feet_Hits[feet_index].bBlockingHit)
				{
					...
					else
					{
						...
						FVector foot_offset_a = extra_foot_offset;
...
*/
	...
	if(SecondUpperLimbIndex.IsValid() && SecondUpperLimbValid && test_counter > 5)
	{
		...
		FVector Impact_Feet_Difference = ((UKismetMathLibrary::InverseTransformLocation(owner_skel_w_transform,feet_impactpoint_array[spine_index][feet_index])) -Original_EndBoneCSTransform.GetLocation())*0.50f;

		Impact_Feet_Difference.Z = FMath::Clamp<float>(Impact_Feet_Difference.Z,0,100000000.0f);
...
//		FVector world_impact_point_CS = owner_skel_w_transform.InverseTransformPosition(feet_impactpoint_array[spine_index][feet_index]);
	...
	if(SecondUpperLimbValid)
	{
		...
		AnimationCore::SolveTwoBoneIK(SecondUpperLimbCSTransform,UpperLimbCSTransform,LowerLimbCSTransform,UpperLimbCSTransform.GetLocation(),LowerLimbCSTransform.GetLocation(),false,1,1);

		OutBoneTransforms.Add(FBoneTransform(SecondUpperLimbIndex, SecondUpperLimbCSTransform));
	}
	...
	if (solve_should_fail == false)
	{
		...
		for (int32 i = 0; i < dragon_input_data.FeetBones.Num(); i++)
		{
			...
			if(is_second_knee_valid)
			 BoneArrayMachine_Feet(i, dragon_input_data.FeetBones[i].Feet_Bone_Name, dragon_input_data.FeetBones[i].Knee_Bone_Name, dragon_input_data.FeetBones[i].Second_Knee_Bone_Name, dragon_input_data.Pelvis, true);
			else
