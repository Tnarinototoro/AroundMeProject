...
#include "DIY_CommuDeviceDelegate.h"

#include "Async/Async.h"
#include "DIY_CommuLog.h"

@implementation DIY_CommuDeviceDelegate
{
}
...
- (void)peripheral:(CBPeripheral*)peripheral didDiscoverServices:(NSError*)error
{
	if (error)
	{
		...
		UE_LOG(DIY_CommuLog, Error, TEXT("Failed to discover services: %s"), *ErrorMessage);
		return;
	}
...
- (void)peripheral:(CBPeripheral*)peripheral didDiscoverCharacteristicsForService:(CBService*)service error:(NSError*)error
{
	...
	{
		const FString ErrorMessage([error localizedDescription]);
		UE_LOG(DIY_CommuLog, Error, TEXT("Failed to discover characteristics: %s"), *ErrorMessage);
		return;
	}
}
