...
(
echo @echo off
echo echo [post-merge] Updating Hooks...
REM --- 改为 start 弹窗运行，/c 表示运行完自动关闭窗口 ---
echo pushd "%%~dp0" ^&^& cd /d "..\..\AroundMe" ^&^& start "Auto Update Hooks" cmd.exe /c "InstallHooks.bat" ^&^& popd
echo echo Launching UE Build Window...
...
(
echo @echo off
echo echo [post-checkout] Updating Hooks...
echo pushd "%%~dp0" ^&^& cd /d "..\..\AroundMe" ^&^& start "Auto Update Hooks" cmd.exe /c "InstallHooks.bat" ^&^& popd
echo echo Launching UE Build Window...
echo start "UE Build" cmd.exe /k ^"^"%BUILD_BAT%^" AroundMeEditor Win64 Development ^"%PROJECT_FILE%^" -WaitMutex ^&^& echo Build OK ^|^| echo Build Failed^"
echo exit /b 0
...
(
echo @echo off
echo echo [pre-push] Updating Hooks...
echo pushd "%%~dp0" ^&^& cd /d "..\..\AroundMe" ^&^& start "Auto Update Hooks" cmd.exe /c "InstallHooks.bat" ^&^& popd
echo echo Launching UE Build Window...
echo start "UE Build" cmd.exe /k ^"^"%BUILD_BAT%^" AroundMeEditor Win64 Development ^"%PROJECT_FILE%^" -WaitMutex ^&^& echo Build OK ^|^| echo Build Failed^"
echo exit /b 0
